...
eval_freq=10000
reward_mode="randomized_proxy_cost"
hardware_profile="balanced"
hardware_cost_coeffs=""
condition_on_hardware=true
hardware_reward_scale=1.0
...
usage() {
    cat <<EOF
Usage: bash train_depth_hw_transfer.sh [OPTIONS]
...
Options:
  ...
  --reward_mode          wall_time, accept_only, proxy_cost, randomized_proxy_cost
  --hardware_profile     Hardware proxy profile (default: $hardware_profile)
  --hardware_cost_coeffs Optional KEY=VALUE coefficient overrides
  --condition_on_hardware / --no_condition_on_hardware
                         Append/disable hardware profile observation features
  --hardware_reward_scale Reward multiplier (default: $hardware_reward_scale)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --hardware_profile)   hardware_profile="$2";     shift 2 ;;
        --hardware_cost_coeffs) hardware_cost_coeffs="$2"; shift 2 ;;
        --condition_on_hardware) condition_on_hardware=true; shift 1 ;;
        --no_condition_on_hardware) condition_on_hardware=false; shift 1 ;;
        --hardware_reward_scale) hardware_reward_scale="$2"; shift 2 ;;
...
set -- \
    ...
    --hardware_reward_scale "${hardware_reward_scale}" \
    "$@"

if [[ -n "${hardware_cost_coeffs}" ]]; then
    set -- --hardware_cost_coeffs "${hardware_cost_coeffs}" "$@"
fi

if [[ "${condition_on_hardware}" == "true" ]]; then
    set -- --condition_on_hardware "$@"
else
    set -- --no_condition_on_hardware "$@"
fi
...
python3 -m rl.rl_depth_hw_transfer \

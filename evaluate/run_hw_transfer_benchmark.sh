...
PYTHON_SCRIPT="${SCRIPT_DIR}/benchmark_hw_transfer.py"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --hardware_profile TEXT Hardware profile for hardware-conditioned policies"
            echo "  --hardware_cost_coeffs TEXT Optional KEY=VALUE coefficient overrides"
...
CMD+=(--hardware_profile "$HARDWARE_PROFILE")

if [ -n "$HARDWARE_COST_COEFFS" ]; then
    CMD+=(--hardware_cost_coeffs "$HARDWARE_COST_COEFFS")
fi

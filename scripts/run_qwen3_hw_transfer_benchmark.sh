...
usage() {
    cat <<EOF
Usage: bash scripts/run_qwen3_hw_transfer_benchmark.sh [extra benchmark args...]
...
Environment:
  ...
  POLICY_LABELS        Optional. Defaults to "bf16_hwproxy".
  ...
  HARDWARE_PROFILE     Optional. Defaults to BENCHMARK_GPU_TAG.
  HARDWARE_COST_COEFFS Optional. Coefficient overrides passed to the copied benchmark.
  RUN_ANALYSIS         Optional. Defaults to 1.

Known policy labels:
  bf16_hwproxy checkpoints/hardware_transfer/Qwen3-14B/bf16_hwproxy/{size,depth}/final.zip
...
resolve_policy_paths() {
    local label="$1"
    case "${label}" in
        bf16_hwproxy|hwproxy|hardware_proxy)
            POLICY_TRAIN_GPU="randomized_proxy"
            SIZE_MODEL_PATH="${HWPROXY_SIZE_MODEL_PATH:-${REPO_ROOT}/checkpoints/hardware_transfer/${MODEL_NAME}/bf16_hwproxy/size/final.zip}"
            DEPTH_MODEL_PATH="${HWPROXY_DEPTH_MODEL_PATH:-${REPO_ROOT}/checkpoints/hardware_transfer/${MODEL_NAME}/bf16_hwproxy/depth/final.zip}"
            ;;
        h100_bf16|h100)
        ...
        a5090_bf16|a5090)
            ...
            ;;
        *)
            echo "Unknown POLICY_LABEL='${label}'. Known labels: bf16_hwproxy, h100_bf16, a5090_bf16." >&2
            exit 1
            ;;
    esac
}
...
HARDWARE_PROFILE="${HARDWARE_PROFILE:-${BENCHMARK_GPU_TAG}}"
...
for eval_quantization in "${QUANT_ARRAY[@]}"; do
    ...
    for policy_label in "${POLICY_ARRAY[@]}"; do
        ...
        bash "${REPO_ROOT}/evaluate/run_hw_transfer_benchmark.sh" \

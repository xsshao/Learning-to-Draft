...
from rl.hardware_reward import (
    COEFF_KEYS,
    hardware_profile_vector,
    infer_policy_obs_size,
    profile_to_log_dict,
    proxy_reward,
...
parser.add_argument(
    "--reward_mode",
    type=str,
    choices=["wall_time", "accept_only", "proxy_cost", "randomized_proxy_cost"],
    default="randomized_proxy_cost",
    help="Hardware-transfer study reward. Use wall_time to reproduce the legacy timed reward.",
)
parser.add_argument(
    "--hardware_profile",
    type=str,
    default="balanced",
    help="Hardware proxy profile: balanced, h100, a5090, rtxa6000, launch_bound, memory_bound, or random.",
)
parser.add_argument(
    "--hardware_cost_coeffs",
    type=str,
    default="",
    help="Optional comma-separated coefficient overrides, e.g. draft=0.03,verify=0.08,launch=0.12.",
)
parser.add_argument(
    "--condition_on_hardware",
    action="store_true",
    default=True,
    help="Append normalized hardware-profile coefficients to the policy observation.",
)
parser.add_argument(
    "--no_condition_on_hardware",
    action="store_false",
    dest="condition_on_hardware",
    help="Disable hardware-profile observation features.",
)
parser.add_argument(
    "--hardware_reward_scale",
    type=float,
    default=1.0,
    help="Multiplicative scale for accept_only/proxy_cost rewards.",
)
    ...
    def _on_step(self) -> bool:
        if "infos" in self.locals and self.locals["infos"]:
            for info in self.locals["infos"]:
                ...
                if "proxy_cost" in info:
                    log_data["custom/proxy_cost"] = info["proxy_cost"]
                for key in COEFF_KEYS:
                    info_key = f"hardware/{key}"
                    if info_key in info:
                        log_data[f"custom/{info_key}"] = info[info_key]
...
class SpeculativeDecodingEnv(gym.Env):
    ...
    def __init__(self, model, logits_processor, input_ids_list):
        ...
        self.hardware_feature_size = len(COEFF_KEYS) if args.condition_on_hardware else 0
        self.base_obs_size = 128
        self.base_obs_size_token = 1268
        ...
        self.obs_size = self.base_obs_size + self.hardware_feature_size
        self.obs_size_token = infer_policy_obs_size(
            self.rl_token,
            self.base_obs_size_token + self.hardware_feature_size,
        )
        self.hardware_profile = None
        self.hardware_profile_obs = np.zeros(len(COEFF_KEYS), dtype=np.float32)
        self._refresh_hardware_profile()
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf,
                                            shape=(self.obs_size,),
                                            dtype=np.float32)
        self.observation_space_token = spaces.Box(low=-np.inf, high=np.inf,
                                            shape=(self.obs_size_token,),
                                            dtype=np.float32)
    ...
    def _refresh_hardware_profile(self):
        self.hardware_profile = resolve_hardware_profile(
            profile_name=args.hardware_profile,
            cost_coeffs=args.hardware_cost_coeffs,
            randomize=self.reward_mode == "randomized_proxy_cost",
        )
        self.hardware_profile_obs = np.array(
            hardware_profile_vector(self.hardware_profile),
            dtype=np.float32,
        )

    def _append_hardware_profile(self, obs, base_size):
        if obs.shape[0] >= base_size + len(COEFF_KEYS):
            obs[base_size:base_size + len(COEFF_KEYS)] = self.hardware_profile_obs
        return obs
    ...
    def _get_obs_total_tokens(self):
        ...
        self._append_hardware_profile(obs, self.base_obs_size_token)
        return obs
    ...
    def _get_obs(self):
        ...
        self._append_hardware_profile(obs, self.base_obs_size)
        return obs
    ...
    def step(self, action):
        ...
        if perform_depth_expansion_step:
            ...
            if hasattr(self, 'rl_token') and self.rl_token is not None:
                token_device = next(self.rl_token.parameters()).device
                token_tensor = torch.tensor(self._get_obs_total_tokens(), device=token_device)
                with torch.no_grad():
            ...
            accepted_count = float(self.accept_length + 1)
            proxy_cost_value = 0.0
            if self.reward_mode == "wall_time":
                reward = accepted_count / (self.time * 100 + 1e-6)
            elif self.reward_mode == "accept_only":
                reward = args.hardware_reward_scale * accepted_count
            else:
                depth_steps = float(max(self.cnet_step, 1))
                launch_steps = depth_steps + 2.0
                reward, proxy_cost_value = proxy_reward(
                    accepted_tokens=accepted_count,
                    profile=self.hardware_profile,
                    drafted_tokens=float(_actual_total_tokens),
                    verify_tokens=float(self.finalized_draft_tokens.shape[1]),
                    depth_steps=depth_steps,
                    kv_tokens=accepted_count,
                    launch_steps=launch_steps,
                    reward_scale=args.hardware_reward_scale,
                )
        ...
        info.update({
            ...
            'proxy_cost': proxy_cost_value,
            'hardware_profile_name': self.hardware_profile.name,
        })
        for key, value in profile_to_log_dict(self.hardware_profile).items():
            if key != "profile_name":
                info[f"hardware/{key}"] = value
...
if __name__ == '__main__':
    ...
    print(
        "Hardware-transfer reward config: "
        f"mode={args.reward_mode}, profile={args.hardware_profile}, "
        f"condition_on_hardware={args.condition_on_hardware}, "
        f"coeff_overrides={args.hardware_cost_coeffs or '(none)'}"
    )

...
class SpeculativeDecodingEnv(gym.Env):
    ...
    def __init__(self, model, logits_processor, input_ids_list):
        ...
        self.base_obs_size = 1268
        self.base_obs_size_depth = 128
        self.obs_size = self.base_obs_size + self.hardware_feature_size
        self.obs_size_depth = infer_policy_obs_size(
            self.depth_model,
            self.base_obs_size_depth + self.hardware_feature_size,
        )
    ...
    def _get_obs_depth(self):
        ...
        self._append_hardware_profile(obs, self.base_obs_size_depth)
        return obs
    ...
    def step(self, action):
        ...
        if self.reward_mode == "wall_time":
            reward = accepted_count / (self.time * 100.0 + 1e-6)
        elif self.reward_mode == "accept_only":
            reward = args.hardware_reward_scale * accepted_count
        else:
            depth_steps = float(max(self.random_depth_this_step, self.cnet_step))
